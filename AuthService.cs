    ...
    public class AuthService
    {
        ...
        public string ValidarUsuario(LoginModel login)
        {
            ...
            if (user != null)
            {
                ...
                {
                    return TokenGenerator.GenerateTokenJwt(user); // Pass the user object
                }
            }
            else
            {
                return null; // Indicate authentication failure with null
            }
            return null; // Consistent null return for failure
        }
        ...
        public AdministradorITM BuscarPorUsuario(string usuario)
        {
            return _context.AdministradorITM.FirstOrDefault(a => a.Usuario == usuario);
        }
    }
}